...
Uso: $0 [--production] [--full] [--cache-disk /mnt/4tb] [--help]
  ...
  --cache-disk PATH  Sincronizza la cache sul 4TB dopo la build.
...
check_deps() {
    ...
    local deps=("wget" "curl" "xorriso" "sed" "git" "rsync" "python3" "fdisk" "dd" "sha256sum")
    for dep in "${deps[@]}"; do
...
main() {
    ...
    rebuild_iso
    verify_built_iso
    sync_cache_to_disk   # se --cache-disk: riempi il 4TB con la cache (stesso build)
    ...
    if [ -n "${CACHE_DISK_MNT:-}" ]; then
        echo "Cache gia' copiata sul 4TB (${CACHE_DISK_MNT}/cache) in questo stesso build."
    else
        echo "Cache NON copiata: rilancia con --cache-disk /mnt/4tb, oppure usa scripts/sync-cache-to-disk.sh."
    fi
