...
# AGGIORNATO 2026-07-15 per la nuova struttura di devin_ai_ide (Tauri, archive,
# training runs): senza queste esclusioni rsync sparerebbe sul rig decine di GB
# inutili (node_modules, target Rust, GGUF locali, sandbox di training).
rsync -avz --delete \
    --exclude '.git' \
    --exclude '__pycache__' \
    --exclude '*.pyc' \
    ...
    --exclude 'workspace/_training_runs/' \
    --exclude 'devin/devin_models/' \
    --exclude '*.gguf' \
    --exclude '*.safetensors' \
