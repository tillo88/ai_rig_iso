#!/bin/bash
# =============================================================================
# STAGE 2/4 — CUDA Toolkit + beellama.
...
if [ -x "$SERVER" ]; then
    echo "beellama già installato: $SERVER"
elif [ -x "$CACHE_DIR/llama-prebuilt/bin/llama-server" ]; then
    echo "Installo beellama precompilato tramite move atomico della directory bin..."
    ...
    echo "Prebuilt assente: compilo beellama sul target."
    BUILD_TMP="/opt/llama.cpp.build.$$"
    rm -rf "$BUILD_TMP" "$LLAMA_DIR"
    git clone --depth 1 "https://github.com/Anbeeld/beellama.cpp.git" "$BUILD_TMP"
    cd "$BUILD_TMP"
