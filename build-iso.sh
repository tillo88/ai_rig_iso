...
check_nccl_cache() {
    step "Verifico runtime NCCL offline per CUDA 12.8..."
    local pkg_dir="${CACHE_DIR}/packages" nccl_deb
    nccl_deb="$(find "$pkg_dir" -maxdepth 1 -type f -name 'libnccl2_*+cuda12.8_amd64.deb' -print -quit 2>/dev/null || true)"
    if [ -z "$nccl_deb" ]; then
        strict_fail_or_warn "Pacchetto NCCL offline assente in $pkg_dir. Esegui: sudo scripts/prepare-nccl-cache.sh '$CACHE_DIR'"
        return 0
    fi
    [ -s "$pkg_dir/SHA256SUMS" ] \
        || { strict_fail_or_warn "Manifest $pkg_dir/SHA256SUMS assente"; return 0; }
    if ! (cd "$pkg_dir" && sha256sum -c SHA256SUMS); then
        error "Checksum pacchetti NCCL non valido in $pkg_dir"
    fi
    [ -x scripts/install-nccl-runtime.sh ] \
        || error "scripts/install-nccl-runtime.sh assente o non eseguibile"
    [ -x scripts/install-nccl-and-resume.sh ] \
        || error "scripts/install-nccl-and-resume.sh assente o non eseguibile"
    info "NCCL offline pronta: $(basename "$nccl_deb")"
}
